...
local function load_dummy_icon()
    local icon_path = ('%s\\resources\\status\\dummy.png'):fmt(AshitaCore:GetInstallPath());
...
local function load_status_icon_from_theme(status_id, theme)
    ...
    T{'.png', '.jpg', '.jpeg', '.bmp'}:forieach(function(ext, _)
        ...
        icon_path = ('%s\\resources\\status\\%s\\%d'):append(ext):fmt(AshitaCore:GetInstallPath(), theme, status_id);
...
statusIcons.get_status_theme_paths = function()
    local path = ('%s\\resources\\status\\'):fmt(AshitaCore:GetInstallPath());
